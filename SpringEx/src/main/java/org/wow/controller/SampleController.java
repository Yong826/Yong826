...
import org.wow.model.SampleVO;

@Controller
public class SampleController {
	// 비동기식은 동기식과는 달리 jsp가 없으므로, 순수한 데이터(주로 많이 사용되는 형태는 문자열,JSON,XML)를 반환합니다.
	// 기존 Controller에서는 문자열을 반환하는 결우 jsp파일의 이름으로 처리하지만 비동기식 Controller의 경우에는 순수한 데이터를 처리한다.
	// 이때 메서드의 리턴타입이 ResponseRntity 객체이면 비동기식으로 처리함.
	...
	public ResponseEntity<SampleVO> getSample1(@RequestBody SampleVO sample){
		...
		System.out.println("sampleno data....="+sample);
